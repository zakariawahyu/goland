...
and ps.kode_barang = br.kode_barang and ps.kode_layanan = ly.kode_layanan and username_laundry='$nama' order by kode_pesanan desc";
            ...
            while ($row = mysqli_fetch_array($hasil2)) {
              ...
              <tr>
                ...
                if ($row['status_pesanan'] == 'Selesai') {
                  ?>
                  <td><button type="button" class="btn btn-default" disabled>Proses</button></td>
                  <?php
                } else if($row['status_pesanan'] == 'Sedang Diproses') {
                  ?>
                  <td><button type="button" class="btn btn-default" disabled>Proses</button></td>
                  <?php
                } else {
               ?>
                <td>
                  <button type="submit" class="btn btn-default" name="proses"><a href="proses.php?id=<?php echo $row['kode_pesanan']; ?>">Proses
                  </a></button>
                </td>
                <?php
              } ?>
